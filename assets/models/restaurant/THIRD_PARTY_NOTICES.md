...
`ar/turkish-shish-kebab.glb`, `ar/turkish-adana-kebab.glb` и `ar/turkish-lentil-soup.glb` созданы непосредственно для меню Alba Space воспроизводимым скриптом `scripts/create-restaurant-turkish-dishes-highpoly.mjs`. Это самостоятельные high-detail glTF 2.0-модели в физическом масштабе: шиш-кебаб состоит из трёх шампуров с мясом и запечёнными овощами, адана-кебаб дополнен питой, овощами и луком с сумахом, чечевичный суп подаётся в собственной керамической миске с лимоном и зеленью. Сетки содержат соответственно 116 128, 148 864 и 164 000 треугольников. Неоднородное запекание, корочка, блики, цвет, шероховатость и металлические свойства встроены в геометрию, вершинные цвета и PBR-материалы; внешние текстуры и сетевые ресурсы не требуются. Модели не являются производными от скачанных сторонних 3D-файлов.

Картинки `assets/images/restaurant/menu/*.webp` используются одновременно как миниатюры карточек и загрузочные постеры. Девять из десяти изображений воспроизводимо отрисованы из фактических GLB скриптом `scripts/render-restaurant-posters.py`. Превью `strawberry-chocolate-cake.webp` получено из официального CC0 preview модели Poly Haven, указанной ниже.
