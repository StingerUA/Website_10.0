...
`ar/turkish-shish-kebab.glb`, `ar/turkish-adana-kebab.glb` и `ar/turkish-lentil-soup.glb` созданы непосредственно для Alba Space воспроизводимым скриптом `scripts/create-restaurant-turkish-dishes-highpoly.mjs`. Шиш-кебаб и адана-кебаб снова используются как первые две позиции мясного меню; мобильные и AR-варианты подаются на встроенной фарфоровой тарелке. Сетки содержат соответственно 116 128, 148 864 и 164 000 треугольников. Неоднородная прожарка, корочка, блики, цвет, шероховатость и металлические свойства встроены в геометрию, вершинные цвета и PBR-материалы; внешние сетевые ресурсы им не требуются.
...
В репозитории также сохранены десять локальных GLB-моделей, ранее скачанных из public gallery Tripo. Страницы Tripo показывали бесплатную загрузку и GLB/GLTF-экспорт; исходные локальные файлы декодированы из `EXT_meshopt_compression`. В каталоге `assets/models/restaurant/ar/` находятся производные WebAR-копии с Draco-сжатием и физическим масштабом в метрах: 0,28 для мясных блюд и 0,56 для остальных. Первые две позиции меню теперь используют высокополигональные турецкие модели `turkish-shish-kebab-plated.glb` и `turkish-adana-kebab-plated.glb`; `realistic-steak-board.glb` и `realistic-steak-slices.glb` оставлены как локальные архивные варианты. Локальное хранение означает, что во время показа блюда страница не зависит от CDN Tripo.
...
## Архивная дневная панорама ресторана

`assets/images/restaurant/warm-restaurant-day-2k.jpg` — оптимизированная до 2048×1024 локальная копия HDRI-панорамы [Warm Restaurant](https://polyhaven.com/a/warm_restaurant) от Poly Haven. Исходник распространяется по [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/). Панорама и прежняя A-Frame-сцена оставлены в репозитории как архив, но текущая ПК-версия, как и мобильная, показывает блюдо через `<model-viewer>` без виртуального стола.

## Runtime dependencies

На мобильных устройствах и компьютерах маршрут использует локальную копию [Google `<model-viewer>`](https://modelviewer.dev/) 3.0.0 (Apache-2.0). На совместимом телефоне доступны WebXR и резервные режимы Android Scene Viewer / Apple Quick Look; на ПК остаётся обычный интерактивный 3D-просмотр и видимая кнопка AR с сообщением о необходимости совместимого устройства. WebXR использует стандартный hit-test поверхности и оценку освещения; фотография стола и серверное распознавание не применяются.
