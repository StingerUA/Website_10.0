...
`realistic-porcelain-plate.glb` — мобильная glTF 2.0-адаптация открытой модели [`bullet3/data/dinnerware/plate.obj`](https://github.com/bulletphysics/bullet3/blob/master/data/dinnerware/plate.obj). Сохранён измеренный профиль исходной посуды, радиальная сетка сглажена, а материал заменён на компактный PBR-фарфор без внешних текстур. Исходный набор `bullet3/data` распространяется с [разрешительной zlib-подобной лицензией Bullet](https://github.com/bulletphysics/bullet3/blob/master/data/LICENSE.txt); адаптация явно обозначена как изменённая версия. Модель тарелки используется под двумя первыми мясными блюдами и десертом с лесными ягодами.

Тарелка встроена в мобильные производные `ar/realistic-steak-board-plated.glb`, `ar/realistic-steak-slices-plated.glb` и `ar/realistic-layered-dessert-cup-plated.glb`. Благодаря этому подача остаётся частью сцены в WebXR, Android Scene Viewer и Apple Quick Look. Производные воспроизводимо собираются скриптом `scripts/compose-restaurant-serving-plates.mjs`; исходные модели блюд сохранены без замены.
