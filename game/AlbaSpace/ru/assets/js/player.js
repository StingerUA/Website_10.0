...
function me(r=room()){return r?.players.find(p=>p.id===playerId)}
function store(code,id){roomCode=code;playerId=id;sessionStorage.setItem("as_player_room",code);sessionStorage.setItem("as_player_id",id)}
function renderHud(r,p){hud.innerHTML=p?`<span class="kpi">💰 ${p.credits}</span><span class="kpi">🛰️ ${p.small+p.large}/10</span><span class="kpi">🎓 ${p.graduates}</span>`:"";phaseLabel.textContent=r?`${r.phase} · ${p?.company||"Игрок"}`:"Вход"}
function render(){const r=room(),p=me(r);renderHud(r,p);if(!r||!p){renderJoin();return}if(!p.company){renderCompany(r,p);return}if(p.cadets.length<3){renderCadetSetup(r,p);return}if(r.phase==="LOBBY"){renderWaiting(r,p);return}if(r.phase==="QUESTION"){renderQuestion(r,p);return}if(r.phase==="RESULT"){renderResult(r,p);return}if(r.phase==="ENDGAME"){renderEnd(r,p);return}renderStation(r,p)}
function renderJoin(){app.innerHTML=`<section class="card"><div class="phase">Вход в комнату</div><h1>Присоединиться 🚀</h1><label class="muted">Код учителя</label><input id="code" class="input big-input" inputmode="numeric" maxlength="5" placeholder="48271"><div style="margin-top:14px"><button id="join" class="btn primary">Войти</button></div></section>`;document.getElementById("join").onclick=()=>{try{const code=document.getElementById("code").value.trim();const p=AS.joinRoom(code);store(code,p.id);render()}catch(e){toast(e.message)}}}
