...
(function () {
  const TOPIC = {
    PLANETS: { color: "#b58cff", icon: "✦", label: "Планеты" },
    SATELLITES: { color: "#66b7ff", icon: "◈", label: "Спутники" },
    TELESCOPES: { color: "#71e0a1", icon: "◉", label: "Телескопы" },
    ROVERS: { color: "#ffad66", icon: "◌", label: "Роверы" },
    TURKISH_SATELLITES: { color: "#ffd166", icon: "★", label: "Турецкие спутники" }
  };
  ...
  class Station3DRenderer {
    ...
    init() {
      ...
      if (!window.BABYLON || !this.host) {
        this.onError(new Error("Babylon.js не загрузился"));
        return false;
      }
      try {
        this.host.innerHTML = "<canvas class=\"station-canvas\" aria-label=\"3D космическая станция\"></canvas><div class=\"station-fallback hidden\">3D режим недоступен на этом устройстве.<br><button class=\"btn ghost\" data-reset-3d>Повторить</button></div>";
    ...
    buildCadet(module, local, cadet) {
      ...
      this.label(root, cadet.name || TOPIC[cadet.topic]?.label || "Кадет", new BABYLON.Vector3(0, 0.83, 0));
    }
