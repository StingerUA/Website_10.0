...
function visibleCounterIndex(context){for(const n of document.querySelectorAll('.section-title span,.q-count,[class*="count"]')){const m=String(n.textContent||'').match(/\b(10|[1-9])\s*\/\s*10\b/);if(m)return Number(m[1])-1;}const m=String(context?.parentElement?.textContent||context?.textContent||'').match(/\b(10|[1-9])\s*\/\s*10\b/);return m?Number(m[1])-1:-1;}
...
function findFlash(host){if(!host)return null;const nodes=[host,...host.querySelectorAll('h1,h2,h3,h4,p,div,span,strong')];for(const n of nodes){const t=norm(n.textContent);const e=flashMap.get(t);if(e)return e;}const all=norm(host.textContent);for(const term of flashTerms)if(term.text.length>=5&&all.includes(term.text))return term;return null;}
