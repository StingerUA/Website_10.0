(() => {
  ...
  function launchStream(launch) {
    return launch?.streamUrl || (/spacex/i.test(launch?.provider || "") ? "https://www.youtube.com/@SpaceX/live" : "https://www.youtube.com/@NASA/live");
  }
  ...
  function renderLaunches(launches) {
    ...
    root.innerHTML = launches.map(launch => {
      ...
      return `<article class="oa-card oa-launch"><div class="oa-launch__top"><span>${when ? `${escape(shortDate.format(new Date(when)))} UTC` : "—"}</span><span class="oa-launch__status">${escape(launch.status || "—")}</span></div><h3>${escape(launch.name || t.mission)}</h3><p>${escape(launch.provider || t.provider)}</p><p class="oa-launch__place">${escape(launch.location || t.location)}</p><strong class="oa-launch__clock" data-countdown="${escape(when)}">—</strong><a class="oa-launch__watch" href="${escape(launchStream(launch))}" target="_blank" rel="noreferrer">${escape(t.watching)}</a></article>`;
    }).join("");
  }
