...
# offline backend: keep cached staff identity/roles
p = Path('cloudflare-worker/pass-offline-backend.js')
...
s = re.sub(
    r"async function isStaff\(env, userId\) \{.*?\n\}\n\nasync function createOfflineTicket",
...
# pass-entry: route dedicated offline API
p = Path('cloudflare-worker/pass-entry.js')
s = p.read_text(encoding='utf-8')
if "pass-offline-backend.js" not in s:
    s = s.replace(
        "import { handlePassRequest } from './pass-backend.js';",
        "import { handlePassRequest } from './pass-backend.js';\nimport { handleOfflinePassRequest } from './pass-offline-backend.js';",
    )
old = """      const user = await getSessionUser(request, env);
      return handlePassRequest(request, env, user, cors);"""
new = """      const user = await getSessionUser(request, env);
      if (url.pathname.startsWith('/api/staff/offline/')) {
        return handleOfflinePassRequest(request, env, user, cors, handlePassRequest);
      }
      return handlePassRequest(request, env, user, cors);"""
if old not in s:
    raise SystemExit('pass-entry routing anchor missing')
s = s.replace(old, new)
p.write_text(s, encoding='utf-8')

# offline runtime: persist cached staff identity
p = Path('assets/js/pass-offline.js')
s = p.read_text(encoding='utf-8')
s = s.replace(
...
marker = "  function normalizeToken(raw){"
if marker not in s:
    raise SystemExit('pass-offline staff identity anchor missing')
s = s.replace(marker, "  function staffIdentity(){return current?.staff||null;}\n\n" + marker)
s = s.replace(
...
# staff UI: online-first, IndexedDB fallback
p = Path('assets/js/pass-staff.js')
s = p.read_text(encoding='utf-8')
s = s.replace(
...
s = re.sub(
    r"  async function lookupToken\(token\)\{.*?\n  \}\n\n  function renderPass",
...
s = re.sub(
    r"  async function redeem\(button\)\{.*?\n  \}\n\n  async function loadPending",
...
s = re.sub(
    r"  async function submitPaymentConfirmation\(event\)\{.*?\n  \}\n\n  function bindSearch",
...
# Staff pages: load offline runtime before staff controller
for filename in ('staff-pass.html', 'eng/staff-pass.html', 'rus/staff-pass.html'):
    p = Path(filename)
    s = p.read_text(encoding='utf-8')
    if '/assets/js/pass-offline.js' not in s:
        s = s.replace(
            '<script src="/assets/js/pass-locale.js" defer></script>',
            '<script src="/assets/js/pass-locale.js" defer></script>\n  <script src="/assets/js/pass-offline.js?v=20260904-offline-v1" defer></script>',
        )
    s = re.sub(r'/assets/js/pass-staff\.js\?v=[^"\']+', '/assets/js/pass-staff.js?v=20260904-offline-v1', s)
    p.write_text(s, encoding='utf-8')

# migration checker
p = Path('scripts/check-pass-migrations.py')
s = p.read_text(encoding='utf-8')
if 'staff_offline_leases' not in s:
    s = s.replace('    "audit_log",\n}', '    "audit_log",\n    "staff_offline_leases",\n}')
s = s.replace('        "0007_add_staff_employees.sql",\n', '        "0007_add_staff_employees.sql",\n        "0008_staff_offline_mode.sql",\n')
p.write_text(s, encoding='utf-8')
...
s = p.read_text(encoding='utf-8')
if 'pass-offline-backend.js' not in s:
    s = s.replace('          node --check cloudflare-worker/pass-entry.js\n', '          node --check cloudflare-worker/pass-entry.js\n          node --check cloudflare-worker/pass-offline-backend.js\n')
if 'assets/js/pass-offline.js' not in s:
    s = s.replace('          node --check assets/js/pass-api.js\n', '          node --check assets/js/pass-api.js\n          node --check assets/js/pass-offline.js\n          node --check sw-pass-staff.js\n')
p.write_text(s, encoding='utf-8')

assert 'handleOfflinePassRequest' in Path('cloudflare-worker/pass-entry.js').read_text(encoding='utf-8')
assert 'AlbaPassOffline' in Path('assets/js/pass-staff.js').read_text(encoding='utf-8')
for filename in ('staff-pass.html', 'eng/staff-pass.html', 'rus/staff-pass.html'):
    assert 'pass-offline.js' in Path(filename).read_text(encoding='utf-8')

print('Offline Staff Mode wiring complete')
